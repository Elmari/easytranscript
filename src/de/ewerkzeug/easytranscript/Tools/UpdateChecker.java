...
import static de.ewerkzeug.easytranscript.Core.V.UPDATECHANNEL;
...
public class UpdateChecker extends SwingWorker<Version, Void> {
    ...
    protected Version doInBackground() {
        ...
        try {
            ...
            String path = "http://e-werkzeug.eu/software/easytranscript/updates/" + VersionState.getString(V.UPDATECHANNEL) + "/version";

            if (!checkIfURLExists(path)) {
                logger.log(Level.WARNING, "{0} version Datei existiert nicht.", VersionState.getString(V.UPDATECHANNEL));
            ...
            if ((V.UPDATECHANNEL != VersionState.getValue(VersionState.FINAL))) {
                website = new URL(path);
                ReadableByteChannel rbc = Channels.newChannel(website.openStream());

                FileOutputStream fos = new FileOutputStream(opFolder+"EWET_version" + VersionState.getString(V.UPDATECHANNEL) + ".txt");
        ...
        try {
            if (new File(opFolder+"EWET_version" + VersionState.getString(V.UPDATECHANNEL) + ".txt").exists()) {
                BufferedReader in2 = new BufferedReader(new FileReader(opFolder+"EWET_version" + VersionState.getString(V.UPDATECHANNEL) + ".txt"));
                ...
                if (r == 1) {
                    new File(opFolder+"EWET_version.txt").delete();
                    new File(opFolder+"EWET_version" + VersionState.getString(V.UPDATECHANNEL) + ".txt").renameTo(new File("version.txt"));
                } else {
                    new File(opFolder+"EWET_version" + VersionState.getString(V.UPDATECHANNEL) + ".txt").delete();
                }
            }
